...
app.whenReady().then(() => {
  ...
  fetchClientInfo(systemInfo,globalUID)
    .then(() => {
      // Send system info after client ID is available
      fetchSystemInfo(systemInfo,globalUID);
      ...
      if (systemInfo.networkInterfaces && systemInfo.networkInterfaces.length > 0) {
          fetchNetworkInfo(systemInfo.networkInterfaces,globalUID);
      }
  })
...
async function fetchClientInfo(systemInfo,globalUID) {
  try {
    const response = await fetch('http://192.168.172.134:5000/api/client-info', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ UID:globalUID,systemData: systemInfo, networkInterfaces: systemInfo.networkInterfaces })
    });
...
async function fetchNetworkInfo(networkInfo, globalUID) {
  if (!globalUID) {
  ...
  try {
    const response = await fetch('http://192.168.172.134:5000/api/network-info', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ globalUID, networkData: networkInfo })
    });

    if (response.ok) {
      console.log('Network info sent successfully');
    } else {
      console.error('Failed to send network info:', response.statusText);
    }
  } catch (error) {
    console.error('Error sending network info:', error);
  }
}
...
function fetchSystemInfo(systemInfo,globalUID) {
  if (!globalUID) {
      console.error('Client ID is not available for system info');
      return;
  }

  fetch('http://192.168.172.134:5000/api/system-info', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ UID:globalUID, systemData: systemInfo })
  })
      .then(() => {
...
function fetchSoftwareInfo() {
  ...
  client.query(query, async function (err, result) {
    ...
    try {
      // await sendSoftwareDataToServer(result);
    } catch (error) {
...
// async function sendSoftwareDataToServer(softwareData) {
//   console.log("The software data is",softwareData.Name);
...
//         'Content-Type': 'application/json',
//       },
//       body: JSON.stringify({ software: softwareData }),
//     });
...
//   } catch (error) {
//     console.error('Error while sending software data:', error);
...
async function collectSystemInfo() {
  try {
    ...
    setTimeout(() => {
    sendDataUniqueToServer(newInfo,globalUID);
    },5000);
...
async function sendDataUniqueToServer(systemInfo,globalUID) {
  ...
  try {
    const response = await fetch('http://192.168.172.134:5000/api/unique_info', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
