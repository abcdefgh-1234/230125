...
app.post('/api/client-info', (req, res) => {
  const { UID,systemData, networkInterfaces, } = req.body;
  ...
  Promise.all(checkDuplicates)
      .then(() => {
          ...
          const insertQuery = `
              INSERT INTO client (UID, hostname, ip_address, mac_address) VALUES ?
          `;
...
app.post('/api/system-info', (req, res) => {
  const { globalUID, systemData } = req.body;
...
const { globalUID, networkData } = req.body;
  if (!UID || !networkData || networkData.length === 0) {
...
// sending the network data to the react app

app.get('/network_data', (req, res) => {
    const query = 'SELECT * FROM network_info'; // Adjust query based on your table
...
// sending the patch data to the react app

app.get('/patch_data', (req, res) => {
    const query = 'SELECT * FROM patch_updates'; // Adjust query based on your table
    db.query(query, (err, results) => {
      if (err) {
        res.status(500).json({ error: err.message });
        return;
...
// sending the system data to the react app

app.get('/system_data', (req, res) => {
    const query = 'SELECT * FROM sys_info'; // Adjust query based on your table
    db.query(query, (err, results) => {
      if (err) {
        res.status(500).json({ error: err.message });
        return;
...
// sending the unique data to the react app

app.get('/unique_data', (req, res) => {
    const query = 'SELECT * FROM unique_info'; // Adjust query based on your table
    db.query(query, (err, results) => {
      if (err) {
        res.status(500).json({ error: err.message });
        return;
...
// sending the anti virus data to the react app

app.get('/antivirus_data', (req, res) => {
    const query = 'SELECT * FROM antivirus_info'; // Adjust query based on your table
    db.query(query, (err, results) => {
      if (err) {
        res.status(500).json({ error: err.message });
        return;
...
// sending the client data to the react app


app.get('/client_data', (req, res) => {
    const query = 'SELECT * FROM client'; // Adjust query based on your table
    db.query(query, (err, results) => {
      if (err) {
        res.status(500).json({ error: err.message });
        return;
